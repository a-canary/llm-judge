...
installable on fresh agent setups. The repo-local `SKILL_reference.md` was
retired in a prior hygiene pass (see git history); the Hermes skill canonical
location is arc-skills. No further sync with this repo is required as part of
this slice.

## Solution

Add a `cc/` plugin manifest that wraps the existing CLI as an installable plugin
following the cc publish flow (`<plugin>@YYYY.MM.DD.HHMMSS` tag). Run the cc
test+publish flow against dev-cc/inbox so the green plugin lands in the
marketplace. No code rewrite — this is a packaging-only slice.
...
2. As a Developer agent, I want the Hermes skill (in arc-skills) to match the current CLI 1:1, so that skill invocations do not fail on stale flag names (e.g. `--rank` vs `--elo-rank`).
...
8. As a Developer agent, I want the Hermes skill to document the keyring + pass + env var credential precedence with current commands, so that fresh installs do not silently fall back to no-key.
...
- Plugin README documents: (a) keyring-based credential lookup, (b) Phase 1 bug fixes (rank_swiss_elo key, compare_fn signature, MiniMax thinking-block strip), (c) `--elo-rank` / `--elo-class` post-positional argument requirement.
...
- Plugin README flag-list audit: diff `--help` output against documented flags; must be 1:1.

## Out of Scope

- Phase 2 EA integration (lives in the trading project, not this repo).
- TypeScript rewrite of run_judge.py (deferred; CHOICES.md still designates Python canonical).
...
- Re-running this slice after any future CLI flag change is required to keep the plugin README and Hermes skill in sync — consider adding a CI lint that diffs `--help` against both docs.
